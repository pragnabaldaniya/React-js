...
module.exports.registerAdmin = async (req, res) => {
    try {
        ...
        if (!req.file) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Profile image is required"));
        }

        console.log(req.file.path);

        const admin = await adminAuthService.fetchSingleAdmin({ email: req.body.email }, true);
        ...
        if (!password) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Password is required"));
        }
...
module.exports.loginAdmin = async (req, res) => {
    try {
        ...
        if (!req.body.email || !req.body.password) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Email and password are required"));
        }
...
module.exports.forgotPassword = async (req, res) => {
    try {
        console.log(req.body);
        if (!req.body.email) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Email is required"));
        }
        ...
        if (admin.attempt_expire && admin.attempt_expire < Date.now()) { // 11:17 < 09:00
            admin.attempt = 0;
        }
        ...
        await adminAuthService.updateAdmin(admin.id || admin._id, { OTP: OTP, OTP_Expire: expireOTPTime, attempt: admin.attempt, attempt_expire: new Date(Date.now() + 1000 * 60 * 60) });
...
module.exports.verifyOTP = async (req, res) => {
    try {
        console.log(req.body);
        if (!req.body.email || req.body.OTP === undefined) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Email and OTP are required"));
        }
        ...
        if (admin.verify_attempt_expire && admin.verify_attempt_expire < Date.now()) { // 11:17 < 09:00
        ...
        if (admin.OTP_Expire && admin.OTP_Expire < Date.now()) { // 09:50 < 09:48
        ...
        await adminAuthService.updateAdmin(admin.id || admin._id, { verify_attempt: admin.verify_attempt, verify_attempt_expire: new Date(Date.now() + 1000 * 60 * 60) });
        ...
        await adminAuthService.updateAdmin(admin.id || admin._id, { OTP: 0, OTP_Expire: null, verify_attempt: admin.verify_attempt, verify_attempt_expire: new Date(Date.now() + 1000 * 60 * 60) });
...
module.exports.newPassword = async (req, res) => {
    try {
        ...
        if (!req.body.email || !req.body.new_password) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, "Email and new password are required"));
        }
        ...
        const updatedPassword = await adminAuthService.updateAdmin(admin.id || admin._id, { password: req.body.new_password });
...
module.exports.fetchSingleAdmin = async (req, res) => {
    try {
        ...
        const singleAdmin = await adminAuthService.fetchSingleAdmin({ _id: req.params.id }, true);

        return res.status(statusCode.OK).json(successResponse(statusCode.OK, false, MSG.ADMIN_SINGLE_FETCH_SUCCESS, singleAdmin));
    } catch (err) {
        console.log("Error : ", err);
        return res.status(statusCode.INTERNAL_SERVER_ERROR).json(errorResponse(statusCode.INTERNAL_SERVER_ERROR, true, MSG.SERVER_ERROR));
    }
...
module.exports.changePassword = async (req, res) => {
    try {
        ...
        const admin = await adminAuthService.fetchSingleAdmin({ _id: req.admin.id || req.admin._id }, false);
        ...
        await adminAuthService.updateAdmin(req.admin.id || req.admin._id, { password: req.body.new_password });
